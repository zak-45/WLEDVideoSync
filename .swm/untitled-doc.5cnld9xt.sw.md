...
title: Untitled doc
---
<SwmSnippet path="/media.py" line="1" collapsed>

---

This code snippet defines a `CASTMedia` class that is used to cast media (such as image or video files) to a DDP device (e.g., WLED). It includes functions for sending multicast images to IP addresses using the DDP protocol. The class has various properties such as `rate`, `stopcast`, `preview`, `scale_width`, `scale_height`, `wled`, `wled_live`, `host`, `port`, and `protocol` that can be configured for the casting process.
...
<SwmSnippet path="/CastAPI.py" line="1">

---

This code snippet is a Python script that defines a class called `CastAPI` and contains various imports and configurations. The `CastAPI` class has an attribute `dark_mode` set to `False`. There is also a FastAPI endpoint defined at `/api` with a function `read_api_root` that returns a JSON response with the status of `WLEDVideoSync`.
