...
const screenModeButton = document.getElementById('screen-capture-btn');
...
function setupWebSocket() {
    ...
    socket = new WebSocket(`wss://${location.host}/ws-mobile`);
...
function streamFrame() {
    ...
    // If a video is ready, stream it
		...
		// Video covers: camera, video files, AND screen stream
...
screenModeButton.addEventListener('click', async () => {
    ...
    try {
        ...
        sourceInfo.textContent = "Screen Share";
        playFileButton.style.display = 'none';
        switchCameraButton.style.display = 'none';

        // Handle when user manually stops screen sharing
        screenStream.getVideoTracks()[0].addEventListener("ended", () => {
            console.log("Screen sharing stopped by user");
            isScreenMode = false;
            video.srcObject = null;
            startVideoStream(currentFacingMode); // return to camera
        });
